...
def xwrise_se_groups(lista, megethos):
...
def ftiakse_dentro(simeia):
    # record_id = grammi sto arxeio, ksekinaei apo 1
    eggrafes = []
    for i in range(len(simeia)):
        rid = i + 1
    ...
    for group in groups_fyllwn:
        node_id = len(oloi_oi_komboi)   # i thesi = to id
    ...
    # ===== EPIPEDA 1, 2, ... mexri riza =====
# Edw DEN kanoume ksana STR.
# Pairnoume tous kombous me ti seira pou parixthisan
# kai tous omadopoioume ana MAX_INT.
    trexon = epipedo_0
    ...
    while len(trexon) > 1:
        ...
        for group in groups_kombwn:
            node_id = len(oloi_oi_komboi)

            entries = []
            mbrs_paidiwn = []
            for paidi in group:
                entries.append((paidi["node_id"], paidi["mbr"]))
...
def grapse_csv(oloi_oi_komboi, path):
    # morfi grammi: node-id , n , flag , (ptr1,geo1) , (ptr2,geo2) , ...
    # flag = 0 gia fylla, 1 gia endiamesous
    f = open(path, "w")

    for kombos in oloi_oi_komboi:
        nid  = kombos["node_id"]
        n    = len(kombos["entries"])
        flag = 0 if kombos["is_leaf"] else 1
